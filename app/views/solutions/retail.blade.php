@extends('layouts.main') 
@section('content') 

        <br><br><br><br><br>
        
            <div class="container text-center">
                <div class="section-title center"><br>
                    <h2>FieldMax® Retail</h2>
                    <div class="line">
                        <hr>
                    </div>
                    <div class="clearfix"></div>
                    <div style="font-size: larger;color: #6F6F6F;">
                        <div class="text-justify">
                            Battle ground for sales and customer acquisition has been changing and so do the way customers buying behavior. Acknowledging this FieldMax® retail application caters to the business requirement of the modern day organisation engaged in retail door to door sales. The application enables the user to make order and sales transaction through a secured gateway. The retail variant of the FieldMax® application provides the user to execute banking transaction for the associated sales transaction executed through mobile application in a secured environment.<br><br>
                            The application has been designed to provide the simplicity of executing mobile transaction without the hassle of stepping out of the user convenience.
                        </div>
                    </div>
                </div>
                
            </div>
            <div class="container text-left">
                <div class="section-title">
                    <h2>Business Benefit:</h2>
                    <div class="line">
                        <hr>
                    </div>
                    <div class="clearfix"></div>
                    <div style="font-size: larger;">
                        <div class="text-justify">
                            <ul class="about-list">
                                <li><span class="fa fa-dot-circle-o"></span>  Limited Inventory holding cost</li>
                                <li><span class="fa fa-dot-circle-o"></span>  Payment can be made instantly</li>
                                <li><span class="fa fa-dot-circle-o"></span>  Sales 24/7 with no physical presence of sales team required</li>
                                <li><span class="fa fa-dot-circle-o"></span>  Low overheads</li>
                                <li><span class="fa fa-dot-circle-o"></span>  Full integration into existing ERP/CRM/Web portal</li>
                            </ul>
                        </div>
                    </div>
                </div>
                
            </div>
            <br><br>

@endsection